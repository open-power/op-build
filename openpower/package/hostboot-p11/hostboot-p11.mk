...
HOSTBOOT_P11_DEPENDENCIES = host-binutils fsp-trace

HOSTBOOT_P11_INSTALL_IMAGES = YES
HOSTBOOT_P11_INSTALL_TARGET = NO

HOSTBOOT_P11_ENV_VARS=$(TARGET_MAKE_ENV) PERL_USE_UNSAFE_INC=1 \
    CONFIG_FILE=$(BR2_EXTERNAL_OP_BUILD_PATH)/configs/hostboot/$(BR2_HOSTBOOT_P11_CONFIG_FILE) \
    OPENPOWER_BUILD=1 CROSS_PREFIX="$(CCACHE) $(TARGET_CROSS)" HOST_PREFIX="" HOST_BINUTILS_DIR=$(HOST_BINUTILS_DIR) \
    HOSTBOOT_VERSION=`cat '$(HOSTBOOT_P11_VERSION_FILE)'` \
    HOSTBOOT_FW_VERSION_SHORT=`cat '$(HOSTBOOT_P11_FW_VERSION_SHORT_FILE)'` \
    HOSTBOOT_FW_VERSION_LONG=`cat '$(HOSTBOOT_P11_FW_VERSION_LONG_FILE)'`

FSP_TRACE_IMAGES_DIR = $(STAGING_DIR)/fsp-trace/

# If BR2_PACKAGE_IBM_FW_PROPRIETARY_P10 is defined then
#  * Include repo ibm-fw-proprietary-p10 as a dependency (HOSTBOOT_P11_DEPENDENCIES)
...
ifeq ($(wildcard $(HOSTBOOT_PRECOMPILED_LIBRARIES)/libecc_static.a),)
    ...
    HOSTBOOT_P11_DEPENDENCIES += ibm-fw-proprietary-p10
...
define HOSTBOOT_P11_BUILD_CMDS
        $(HOSTBOOT_P11_ENV_VARS) bash -c 'cd $(@D) \
...
define HOSTBOOT_P11_INSTALL_IMAGES_CMDS
        cd $(@D) && $(HOSTBOOT_P11_ENV_VARS) source ./env.bash && $(@D)/src/build/tools/hbDistribute --openpower $(STAGING_DIR)/hostboot_build_images/
        cd $(@D) && $(HOSTBOOT_P11_ENV_VARS) source ./env.bash && $(@D)/src/build/tools/hbDistribute --openpower-sim $(STAGING_DIR)/hostboot_sim_data/
