...
P9SO_SIM_XML_VERSION ?= a89100084400694a585273e64f9696b6f3b4da48
P9SO_SIM_XML_SITE ?= $(call github,sannerd,p9so_sim-xml,$(P9SO_SIM_XML_VERSION))

P9SO_SIM_XML_LICENSE = Apache-2.0
P9SO_SIM_XML_DEPENDENCIES = hostboot openpower-mrw common-p8-xml

P9SO_SIM_XML_INSTALL_IMAGES = YES
P9SO_SIM_XML_INSTALL_TARGET = YES
...
P9SO_SIM_BIOS_XML_CONFIG_FILE = $(MRW_SCRATCH)/$(BR2_P9SO_SIM_BIOS_XML_FILENAME)
...
define P9SO_SIM_XML_BUILD_CMDS
        # copy the p9so_sim xml where the common lives
        bash -c 'mkdir -p $(MRW_SCRATCH) && cp -r $(@D)/* $(MRW_SCRATCH)'

        # generate the system mrw xml -- leave this in when real xml is avail
        #perl -I $(MRW_HB_TOOLS) \
        #$(MRW_HB_TOOLS)/processMrw.pl -x $(MRW_SCRATCH)/p9so_sim.xml

        # copy local simics_NIMBUS xml
        cp $(MRW_HB_TOOLS)/simics_NIMBUS.system.xml $(MRW_SCRATCH)/$(BR2_P9SO_SIM_SYSTEM_XML_FILENAME)
        
        # merge in any system specific attributes, hostboot attributes
        $(MRW_HB_TOOLS)/mergexml.sh $(MRW_SCRATCH)/$(BR2_P9SO_SIM_SYSTEM_XML_FILENAME) \
            ...
            $(MRW_SCRATCH)/$(BR2_P9SO_SIM_MRW_XML_FILENAME) > $(MRW_HB_TOOLS)/temporary_hb.hb.xml;
        ...
        $(MRW_HB_TOOLS)/xmltohb.pl  \
            ...
            --bios-xml-file=$(P9SO_SIM_BIOS_XML_CONFIG_FILE) \
