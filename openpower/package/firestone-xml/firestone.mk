...
#
################################################################################
FIRESTONE_XML_VERSION ?= 10b9c2d1b620a005696b69e351bca2664621e9c0
FIRESTONE_XML_SITE ?= $(call github,rjknight,firestone-xml,$(FIRESTONE_XML_VERSION))
...
define FIRESTONE_XML_BUILD_CMDS
        ...
        # run the mrw parsers
        $(FIRESTONE_XML_ENV_VARS) bash -c 'cd $(MRW_SCRATCH) && $(MAKE) firestone'

		# move the APSS xml to the output area
		cp $(MRW_SCRATCH)/FIRESTONE_APSS.xml $(MRW_INSTALL_DIRECTORY)

        # generate the system mrm xml
        $(MRW_HB_TOOLS)/genHwsvMrwXml.pl \
            --system=$(BR2_OPENPOWER_CONFIG_NAME) \
            --mrwdir=$(MRW_INSTALL_DIRECTORY) \
            --build=hb > $(MRW_INSTALL_DIRECTORY)/$(BR2_FIRESTONE_MRW_XML_FILENAME)
endef

define FIRESTONE_XML_INSTALL_IMAGES_CMDS

        # merge in any system specific attributes, hostboot attributes
        $(MRW_HB_TOOLS)/mergexml.sh $(@D)/$(BR2_FIRESTONE_SYSTEM_XML_FILENAME) \
            ...
            $(MRW_INSTALL_DIRECTORY)/$(BR2_FIRESTONE_MRW_XML_FILENAME) > $(MRW_HB_TOOLS)/temporary_hb.hb.xml;
